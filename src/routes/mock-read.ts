...
const offers = [{ id: "mock-offer-001", label: "Fictitious offer 001" }] as const;
const operations = [{ label: "Fictitious operation 001", txHash: MOCK_TRANSACTION_HASH }] as const;
const creditLimits = [{ institutionId: "mock-institution-001", label: "Fictitious institution" }] as const;

const closedQuerySchema = { additionalProperties: false, properties: {}, type: "object" } as const;
...
const operationListResponse = mockEnvelope({ items: operationSchema, type: "array" });
const operationResponse = mockEnvelope(operationSchema);
const creditLimitListResponse = mockEnvelope({ items: creditLimitSchema, type: "array" });
...
export async function registerMockReadRoutes(app: FastifyInstance): Promise<void> {
  app.get(
    "/api/offers",
    { schema: { querystring: closedQuerySchema, response: { 200: offerListResponse, 400: problemSchema }, tags: ["mock reads"] } },
    async (_request, reply) => sendMock(reply, offers),
  );
  app.get<{ Params: { id: string } }>(
    "/api/offers/:id",
    { schema: { params: offerParamsSchema, response: { 200: offerResponse, 400: problemSchema, 404: problemSchema }, tags: ["mock reads"] } },
    async (request, reply) => {
  ...
  app.get(
    "/api/operations",
    { schema: { querystring: closedQuerySchema, response: { 200: operationListResponse, 400: problemSchema }, tags: ["mock reads"] } },
    async (_request, reply) => sendMock(reply, operations),
  );
  app.get<{ Params: { txHash: string } }>(
    "/api/operations/:txHash",
    { schema: { params: transactionParamsSchema, response: { 200: operationResponse, 400: problemSchema, 404: problemSchema }, tags: ["mock reads"] } },
    async (request, reply) => {
      const operation = operations.find((item) => item.txHash === request.params.txHash);
      return operation ? sendMock(reply, operation) : notFound(reply, request.id);
    },
  );
  app.get(
    "/api/credit-limits",
    { schema: { querystring: closedQuerySchema, response: { 200: creditLimitListResponse, 400: problemSchema }, tags: ["mock reads"] } },
